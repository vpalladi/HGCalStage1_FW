...
namespace mp7 {
    ...
    PPRamNode::upload( const std::vector<uint32_t>& frames, uint32_t offset ) {
        ...
        if( frames.size() > ngoodframes ) {
            MP7_LOG(l7::kWarning) << "Frames beyond " << available << " dropped";
        }

        MP7_LOG(l7::kDebug) << "Writing " << ngoodframes << ", " << int(ngoodframes / nrams) << " per RAM";
        ...
        MP7_LOG(l7::kNotice) << frames.size() << " frames loaded";
    }
    ...
    PPRamNode::writeRAM( uint32_t id, const std::vector<uint32_t>& data ) const {
        ...
        MP7_LOG(l7::kNotice) << "Writing to address: " << l7::hexFmt(pp_base_addr + ram_addr) << " offset: " << l7::hexFmt(ram_addr);
        // write!
    ...
    PPRamNode::readRAM( uint32_t id, uint32_t depth ) const {
        ...
        // offset of the ram to write to
        uint32_t ram_addr = id << 10;
        MP7_LOG(l7::kNotice) << "Reading to address: " << l7::hexFmt(pp_base_addr + ram_addr) << " offset: " << l7::hexFmt(ram_addr);
