...
void sdr_poll()
{
  // Here we should read  the hot-swap sensor
...
#if (MINI_T5_REV > 0) || (!defined(MINI_T5_REV))
...
void sdr_set_temp_reading ( U8 pTempSensor, float pTemp )
{
...
#if (MINI_T5_REV > 1 || defined(MP7_REV) || defined(FC7_REV))
  ...
  switch ( gTemp1State )
  {
    case SENSOR_STATE_NORMAL:
      ...
      if ( gTemp1 >= ( ( S8 ) SDR3[SDR_UPPER_NC_OFST] )
           + ( ( S8 ) SDR3[SDR_GH_HYST_OFST] ) )
      {
        xNewState = SENSOR_STATE_ABOVE_NC;
        xEventCode = SENSOR_TRANS_UNC_GH;
      }

      if ( gTemp1 <= ( ( S8 ) SDR3[SDR_LOWER_NC_OFST] )
           + ( ( S8 ) SDR3[SDR_GL_HYST_OFST] ) )
      {
    ...
    case SENSOR_STATE_ABOVE_NC:
      ...
      if ( gTemp1 >= ( ( S8 ) SDR3[SDR_UPPER_C_OFST] )
           + ( ( S8 ) SDR3[SDR_GH_HYST_OFST] ) )
      {
    ...
    case SENSOR_STATE_ABOVE_C:
      ...
      if ( gTemp1 >= ( ( S8 ) SDR3[SDR_UPPER_NR_OFST] )
           + ( ( S8 ) SDR3[SDR_GH_HYST_OFST] ) )
      {
    ...
    case SENSOR_STATE_BELOW_NC:
      ...
      if ( gTemp1 <= ( ( S8 ) SDR3[SDR_LOWER_C_OFST] )
           + ( ( S8 ) SDR3[SDR_GL_HYST_OFST] ) )
      {
    ...
    case SENSOR_STATE_BELOW_C:
      ...
      if ( gTemp1 <= ( ( S8 ) SDR3[SDR_LOWER_NR_OFST] )
           + ( ( S8 ) SDR3[SDR_GL_HYST_OFST] ) )
      {
