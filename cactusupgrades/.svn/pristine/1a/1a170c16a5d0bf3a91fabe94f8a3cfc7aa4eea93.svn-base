...
//
//  
//	Model:		CY7C25632KV18 4M X 18 QDR(tm)-II+ Burst-of-4 SRAM
...
module cyqdr2_b4(TCK,TMS,TDI,TDO,D, Q, A, K, Kb, RPSb, WPSb, BWS0b, BWS1b,CQ, CQb,ZQ,DOFF, QVLD, ODT);
...
reg [19:0] Read_Address_o_o_o_o_o; 
reg [19:0] Write_Address_int; 
...
always @(datahold_clk)
begin
	if(chip_oe == 1) `tcqdoh Data_out = 18'bz;
end
...
begin
	...
	//		$display(" burst 2 ", $time);
		...
		if (rpen_o_o_o_o_o_o == 0) // added to account for extra latency in QDR2+
		begin
...
begin
		if (rpen_o_o_o == 0) // added to account for extra latency in QDR2+
		begin
			Data_out =  `tco mem1[Read_Address_o_o]; // added to account for extra latency in QDR2+
   //		   $display(" burst 1 ", $time);
		end


		if (rpen_o_o_o_o_o == 0) // added to account for extra latency in QDR2+
		begin
			Data_out = `tco mem3[Read_Address_o_o_o_o]; // added to account for extra latency in QDR2+
...
always @(datahold_clk)
begin
	if(tristate == 0) `tcqdoh Data_out = 18'bz; 
end

// QVLD logic as defined by datasheet spec
always @(posedge echo_clk)
begin
   if (rpen_o_o_o == 0 || rpen_o_o_o_o == 0) // added for extra read latency
	begin
		`tQVLD QVLD_reg = 1'b1; // added for QDR-II+
	end
	else
		`tQVLD QVLD_reg = 1'b0; //added for QDR-II+
end
...
begin 
	state = `reset;
	prev_state = `reset;
	vendor_code = 32'b00011010010001010100000001101001;	// IDCODE for QDR2+ CY7C2563KV18 device. This may not be changed.
