                    ...
                    U8* pDataOut, U8* pDataOutLen )
{
  *pDataOutLen = 0x00;
  ...
  if ( pDataInLen != sizeof ( struct IPMI_CMD_OEM_COMMIT_REQ ) )
  {
    return IPMI_CC_REQ_DATA_INV_LENGTH;
  }

  struct IPMI_CMD_OEM_COMMIT_REQ* xRequest =
    ( struct IPMI_CMD_OEM_COMMIT_REQ* ) pDataIn;

  if ( ( xRequest->Magic[0] != OEM_MAGIC0 ) ||
       ( xRequest->Magic[1] != OEM_MAGIC1 ) )
  {
                    ...
                    U8* pDataOut, U8* pDataOutLen )
{
  *pDataOutLen = 0x00;
  ...
  if ( pDataInLen != sizeof ( struct IPMI_CMD_OEM_SETMAC_REQ ) )
  {
    return IPMI_CC_REQ_DATA_INV_LENGTH;
  }

  struct IPMI_CMD_OEM_SETMAC_REQ* xRequest =
    ( struct IPMI_CMD_OEM_SETMAC_REQ* ) pDataIn;
                   ...
                   U8* pDataOut, U8* pDataOutLen )
{
  *pDataOutLen = 0x00;
  ...
  if ( pDataInLen != sizeof ( struct IPMI_CMD_OEM_SETIP_REQ ) )
  {
    return IPMI_CC_REQ_DATA_INV_LENGTH;
  }

  struct IPMI_CMD_OEM_SETIP_REQ* xRequest =
    ( struct IPMI_CMD_OEM_SETIP_REQ* ) pDataIn;
...
{
  *pDataOutLen = 0x00;
  dprintf ( "[OEM] Network Set Net Enable Command!\n" );
                     ...
                     U8* pDataOut, U8* pDataOutLen )
{
  *pDataOutLen = 0x00;
  ...
  struct IPMI_CMD_OEM_GETNETINF_RESP* xResp =
    ( struct IPMI_CMD_OEM_GETNETINF_RESP* ) pDataOut;

  *pDataOutLen = sizeof ( struct IPMI_CMD_OEM_GETNETINF_RESP );
  ...
  fru_get_curr_offset ( xResp->CurrentOffsets );
...
{
  *pDataOutLen = 0x00;
  dprintf ( "[OEM] Network Set Current Offset Command!\n" );

  if ( pDataInLen != sizeof ( struct IPMI_CMD_OEM_SETCURROFFSET_REQ ) )
  {
    return IPMI_CC_REQ_DATA_INV_LENGTH;
  }

  struct IPMI_CMD_OEM_SETCURROFFSET_REQ* xRequest =
    ( struct IPMI_CMD_OEM_SETCURROFFSET_REQ* ) pDataIn;

  fru_set_curr_offset ( xRequest->CurrentOffsets );
