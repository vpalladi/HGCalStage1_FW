...
void fru_load_network()
{
  ...
  if ( gFlashUserSettings->Magic == USER_MAGIC )
  {
    ...
    dprintf ( "Network-settings: IP: %u.%u.%u.%u , MAC: %02uX:%02uX:%02uX:%02uX:%02uX:%02uX\n",
...
void fru_save_network()
{
  ...
  dprintf ( "Network-settings: IP: %u.%u.%u.%u , MAC: %02uX:%02uX:%02uX:%02uX:%02uX:%02uX , RARP: %s\n",
            gMemUserSettings.IPAddr[0], gMemUserSettings.IPAddr[1], gMemUserSettings.IPAddr[2], gMemUserSettings.IPAddr[3],
            gMemUserSettings.MacAddr[0], gMemUserSettings.MacAddr[1], gMemUserSettings.MacAddr[2], gMemUserSettings.MacAddr[3], gMemUserSettings.MacAddr[4], gMemUserSettings.MacAddr[5],
...
void fru_set_net_enable ( U8* pNETENABLE )
{
  if ( *pNETENABLE )
  {
    gMemUserSettings.IPOptions |= IP_OPT_NETENABLE;
  }
  else
  {
   gMemUserSettings.IPOptions &= ~IP_OPT_NETENABLE;
...
void fru_get_net_enable ( U8* pNETENABLE )
{
  *pNETENABLE = ( ( gMemUserSettings.IPOptions & IP_OPT_NETENABLE ) ? 1 : 0 ) ;
}

void fru_set_curr_offset (U8* pCurrOffsets){
	gMemUserSettings.CurrentOffsets[0] = pCurrOffsets[0];
	gMemUserSettings.CurrentOffsets[1] = pCurrOffsets[1];
}

void fru_get_curr_offset (U8* pCurrOffsets){
	pCurrOffsets[0] = gMemUserSettings.CurrentOffsets[0];
	pCurrOffsets[1] = gMemUserSettings.CurrentOffsets[1];
}


bool fru_check(volatile U8* gUserPage, U8* gFRU_Info, int size ){

  int i=0;
  for(i=0; i<size; i++){
    //dprintf("[FRU] gUserPage: %d, gFRUinfo: %d., i=%d\n",gUserPage[i],gFRU_Info[i],i);
    if(gUserPage[i]!= gFRU_Info[i]){
      dprintf("[FRU] User page is corrupt or outdated.\n");
      return false;
    }
  }
  dprintf("[FRU] Flash user page is up-to-date.\n");
  return true;
}
...
void fru_init()
{
  // Check if the User Page has changed 

  // Before checking the user page, update our checksums
  fru_recalc_checksums();
  int size = sizeof(gFRU_Info)/sizeof(gFRU_Info[0]);
  dprintf("size of user page checked: %d\n", size);

  if(!fru_check(gUserPage, gFRU_Info, size)) {
