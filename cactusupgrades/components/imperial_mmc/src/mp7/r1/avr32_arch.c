...
static const gpio_map_t sGPIO_MAP_12V_UP =
{
...
void arch_init()
{
  ...
  AVR32_SMC.cs[1].pulse = 0x02020202; // 1 cycle hold for everything -- ADDED 1 cycle to NRD and NWE to compensate for fpga pipe buffer
  AVR32_SMC.cs[1].cycle = 0x00030003; // 3 (clock-)cycle (memory-)cycle time -- ADDED 1 cycle to NRD and NWE to compensate for fpga pipe buffer
...
void gpio_enable_12V_up()
{
	 ...
	 // And make sure we're not resetting the FPGA
	  ...
	  gpio_enable_module ( sGPIO_MAP_12V_UP,
	                        sizeof ( sGPIO_MAP_12V_UP ) / sizeof ( sGPIO_MAP_12V_UP[0] ) );

	  return;

}

void gpio_reset_12V_up(){

	gpio_disable_module( sGPIO_MAP_12V_UP,
			sizeof ( sGPIO_MAP_12V_UP ) / sizeof ( sGPIO_MAP_12V_UP[0] ) ) ;


	gpio_disable_pin_pull_up ( USB_GPIO_ID );

	gpio_local_disable_pin_output_driver ( FPGA_GPIO_RESET );
	gpio_local_disable_pin_output_driver ( FPGA_PROG_B );
	gpio_local_disable_pin_output_driver ( FPGA_INIT_B );
	gpio_local_disable_pin_output_driver ( FPGA_DONE );
	gpio_local_disable_pin_output_driver ( FPGA_IPBUS_DONE );
	gpio_local_disable_pin_output_driver ( FPGA_IPBUS_NEW );
	gpio_local_disable_pin_output_driver (TWI_SENS_MPLEX_RST);
